<template>
  <main class="pl-[136px]">
    <div class="text bg-unify-purple-500 mt-[-50px] pl-140px pt-[114px] radius">
      <h3 class="text-white text-[32px] leading-10">How it works</h3>
      <div class="grid grid-cols-2">
        <div>
          <div class="flex items-center mt-35px">
            <h4 class="text-78px text-unify-purple-200">01</h4>
            <p class="ml-7 text-20px text-white">Request demo</p>
          </div>
          <div class="flex items-center my-[35px]">
            <h4 class="text-78px text-unify-purple-200">02</h4>
            <p  class="ml-7 text-20px text-white">Download the app</p>
          </div>
          <div class="flex items-center">
            <h4 class="text-78px text-unify-purple-200">03</h4>
            <p class="ml-7 text-20px text-white">Onboard your students</p>
          </div>
        </div>
        <div>
          <img src="../assets/landing/How-it-works.png" alt="" />
        </div>
      </div>
    </div>
  </main>
</template>

<script setup></script>

<style scoped>
.radius {
  border-radius: 20px 0px 0px 0px;
}
</style>
